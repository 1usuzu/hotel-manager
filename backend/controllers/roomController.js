...
exports.searchRooms = async (req, res) => {
  try {
    ...
    if (minPrice && maxPrice && Number(minPrice) > Number(maxPrice)) {
      return res
        .status(400)
        .json({ error: 'Giá tối thiểu phải nhỏ hơn giá tối đa' })
    }

    // Lọc theo tình trạng: loại phòng đang bảo trì
    // Không filter 'booked' vì phòng có thể được đặt cho nhiều khoảng thời gian khác nhau
    whereRoom.status = { [Op.ne]: 'maintenance' }
...
exports.getRecommendedRooms = async (req, res) => {
  try {
    ...
    const whereRoom = {
      status: { [Op.ne]: 'maintenance' }, // Chỉ loại phòng đang bảo trì
    }
    ...
    return res.status(500).json({ error: 'Lỗi server khi gợi ý phòng' })
  }
}
